...
function _recurseSchema(inputITC, sourceITC, opts, indexedFields, pathName) {
    ...
    sourceITC.getFieldNames().forEach((fieldName) => {
        ...
        if (sourceITC === fieldTC)
            return;
        const baseTypeName = `${opts.baseTypeName}${utils_1.upperFirst(fieldName)}`;
        ...
        if (fieldTC instanceof graphql_compose_1.ScalarTypeComposer || fieldTC instanceof graphql_compose_1.EnumTypeComposer) {
            if (fieldOperatorsConfig &&
                ...
                throw new Error(`You provide incorrect operators config for field '${opts.baseTypeName}.${fieldName}'. This field has Scalar type, so you may provide array or false. Received: ${graphql_compose_1.inspect(fieldOperatorsConfig)}`);
            }
...
function _createOperatorsField(itc, model, opts) {
    ...
    const indexedFields = utils_1.getIndexesFromModel(model).map((o) => Object.keys(o)[0]);
